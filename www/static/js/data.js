...
(function () { // ES module
  ...
  ES.openReportsChildRow = function (data, id, type) {
    ...
    if (data.genome_report === undefined && data.methylome_report === undefined) {
      $(reports_html).find('#child_template_body').text('There is no PGP-UK reports currently associated with this participant.');
    } else {
      $(reports_html).find('#child_template_body').append(
        $('<p id="genome_reports">'),
        $('<p id="methylome_reports">'),
      )
  ...
  ES.addReports = function (data, child_html, type, wrapper_id) {
    ...
    for (var i = 0; i < data[type].length; i++) {
      $('<a download="">').addClass('btn btn-lg btn-default')
        .css('margin-right', '5px').attr('type', 'button')
