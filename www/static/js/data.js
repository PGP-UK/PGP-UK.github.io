...
(function() {
  ...
  PGP.init = function() {
    $.getJSON('/data/json/table.json', function(json) {
      PGP.table_json = json;
      var datatable = PGP.initializeTable(json, 'data_table');
      $.getJSON('/api/' + PGP.API_VERSION + '/all_participants.json', function(data_json) {
        PGP.data_json = data_json;
        PGP.makeIconClickable(datatable, 'data_table', data_json, '.file_btn');
        PGP.makeIconClickable(datatable, 'data_table', data_json, '.report_btn');
        PGP.makeIconClickable(datatable, 'data_table', data_json, '.trait_btn');
      });
    });
  };

  PGP.initializeTable = function(dataset, tableId) {
  ...
  PGP.makeIconClickable = function(datatable, tableId, dataset, type) {
    ...
    $('#' + tableId).on('click', type, function() {
      ...
      var row = datatable.row($tr);
      PGP.openChildRow(row, $tr, dataset, id, type);
    });
  };
  ...
  PGP.openReportsChildRow = function(data, id, type) {
    ...
    if (
      ...
      PGP.addReports(data, reports_html, 'methylome_report', '#methylome_reports');
    }
    return reports_html;
  };
